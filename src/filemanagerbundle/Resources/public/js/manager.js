...
$(function () {
    ...
    var $generatefileModal = $('#js-confirm-generatefile');
    ...
    var callback = function (key, opt) {
        switch (key) {
            ...
            case 'preview':
                ...
                break;
            case 'generatefile':
                var $generatefileModalButton = opt.$trigger.find(".js-generatefile-modal")
                generateFile($generatefileModalButton)
                $displayModal.modal("show");
                break;
        }
    };
    ...
    $.contextMenu({
        ...
        callback: callback,
        items: {
            "delete": {name: deleteMessage, icon: ''},
            "edit": {name: renameMessage, icon: ''},
            "download": {name: downloadMessage, icon: ''},
        }
    });
    $.contextMenu({
        selector: '.img',
        ...
        items: {
            ...
            "edit": {name: renameMessage, icon: ''},
            "download": {name: downloadMessage, icon: ''},
            "preview": {name: previewMessage, icon: ''},
        }
    });
    $.contextMenu({
        selector: '.dir',
    ...
    function generateFile($generatefileModalButton) {
        $('#js-confirm-generatefile').find('form').attr('action', $generatefileModalButton.data('href'));
    }
    ...
    $(document)
        ...
        // generate file modal buttons
        .on('click', '.js-generatefile-modal', function () {
                generateFile($(this));
            }
        )
